...
export default function ItemOrders(props: any) {
  ...
  const [nextPageProduct, setNextPageProduct] = useState<QueryDocumentSnapshot<DocumentData, DocumentData> | null>();
  ...
  const handleLoadProducts = async (currentProducts: any = [], next: any = null) => {
  ...
  const handleScroll = (e: any) => {
    ...
    if (target.scrollTop + target.offsetHeight >= target.scrollHeight && (products.length < totalProducts)) {
      setLoadBottom(true)
  ...
  const handleQuantityChange = (value: any, index: any) => {
    ...
    let currentItems = form.getFieldValue('items');
    let productIndex = products.findIndex((product: any) => product.value === currentItems[index].id);

    currentItems[index] = {
      ...currentItems[index],
      price: products[productIndex].cost * value
    }
    ...
    <Form.List name="items">
      {(fields, { add, remove }) => (
        <>
              ...
              {fields.map(({ key, name, ...restField }, index) => (
                <tr key={index} className={styles.table_row}>
                  <td className={styles.name_column}>
                    <Form.Item {...restField} name={[name, 'id']} initialValue="">
                      <Select 
                  ...
                  <td>
                    <Form.Item 
                      ...
                      initialValue={0}
                      hasFeedback={true}
                      rules={[{ type: 'number', required: true, min: 1, message: '' }]}
                    >
                      <InputNumber 
                        bordered={false} 
                        min={0} 
                  ...
                  <td>
                    <Form.Item {...restField} name={[name, 'price']} initialValue={0}>
                      <Input 
                        bordered={false} 
                        readOnly 
                        min={0} 
                        value={form.getFieldsValue(true).items[index]? form.getFieldsValue(true).items[index].quantity : 0}  
                      />
                    </Form.Item>
                  </td>
          ...
          <Button onClick={() => handleAddItem(add)} className={styles.add_button} type="primary" block>
            Add Item
          </Button>
        </>
      )}
